...
public class Data {
  ...
  public static ArrayList<Integer> getNotesBySearch(String username, String search) {
    ArrayList<ArrayList<String>> data =
        Database.execute(
            "SELECT id FROM notes WHERE username = ? AND (text LIKE ? OR heading LIKE ?)",
            new String[] {username, "%" + search + "%", "%" + search + "%"},
            false);
  ...
  public static ArrayList<String> getNoteById(int id) {
    return Database.execute(
            "SELECT id, username, heading, text FROM notes WHERE id = ?",
  ...
  public static int addNote(String username, String heading, String text) {
    Database.execute(
        "INSERT INTO notes (username, heading, text) VALUES (?, ?, ?)",
        new String[] {username, heading, text},
        true);
    return Integer.parseInt(
        Database.execute("SELECT last_insert_rowid()", new String[0], false).get(0).get(0));
  }

  public static void updateNoteHeading(int id, String heading) {
    Database.execute(
        "UPDATE notes SET heading = ? WHERE id = ?",
        new String[] {heading, Integer.toString(id)},
        true);
  }

  public static void updateNoteText(int id, String text) {
    Database.execute(
        "UPDATE notes SET text = ? WHERE id = ?", new String[] {text, Integer.toString(id)}, true);
  }
  ...
  public static ArrayList<String> getTagsByNoteId(int noteId) {
    ...
    for (ArrayList<String> tag :
        Database.execute(
            "SELECT tag FROM tags WHERE noteID = ?",
  ...
  public static void addTagToNoteId(int noteId, String tag) {
    Database.execute(
        "INSERT INTO tags (noteID, tag) VALUES (?, ?)",
  ...
  public static void removeTagFromNoteId(int noteId, String tag) {
    Database.execute(
        "DELETE FROM tags WHERE noteID = ? AND tag = ?",
        new String[] {Integer.toString(noteId), tag},
        true);
  }
  ...
  public static ArrayList<Integer> getNotesByUsername(String username) {
    ...
    for (ArrayList<String> note :
        Database.execute(
            "SELECT DISTINCT id FROM notes WHERE username = ?", new String[] {username}, false)) {
      notes.add(Integer.parseInt(note.get(0)));
    }
    return notes;
  }
  ...
  public static ArrayList<String> getUserByUsername(String username) {
    return Database.execute(
            "SELECT * FROM users WHERE username = ?", new String[] {username}, false)
        .get(0);
  }

  public static void addUser(
      String username, String displayName, String passwordSalt, String passwordHash) {
    Database.execute(
        "INSERT OR IGNORE INTO users (username, displayName, passwordSalt, passwordHash) VALUES (?,"
            + " ?, ?, ?)",
        new String[] {username, displayName, passwordSalt, passwordHash},
        true);
  }
  ...
  public static void updateUserDisplayName(String username, String displayName) {
    Database.execute(
        ...
        new String[] {displayName, username},
        true);
  }

  public static void updateUserPassword(String username, String passwordSalt, String passwordHash) {
    Database.execute(
        "UPDATE users SET passwordSalt = ?, passwordHash = ? WHERE username = ?",
        new String[] {passwordSalt, passwordHash, username},
        true);
  }
  ...
  public static void addPermission(int noteID, String username, boolean permissionMode) {
    Database.execute(
        "INSERT INTO permissions (noteID, username, permissionMode) VALUES (?, ?, ?)",
        new String[] {Integer.toString(noteID), username, permissionMode ? "1" : "0"},
        true);
  }
  ...
  public static void changePermission(boolean permissionMode, int noteID, String username) {
    Database.execute(
        ...
        new String[] {permissionMode ? "1" : "0", Integer.toString(noteID), username},
        true);
  }

  public static ArrayList<ArrayList<String>> getSharedNotesByUsername(String username) {
    return Database.execute(
        "SELECT noteID, permissionMode FROM permissions WHERE username = ?",
        new String[] {username},
        false);
  }
