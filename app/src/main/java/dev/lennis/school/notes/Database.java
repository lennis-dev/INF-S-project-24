...
public class Database {
  ...
  public static void install() {
    String[] sql = {
      ...
      "CREATE TABLE IF NOT EXISTS users (username TINYTEXT PRIMARY KEY, displayName TINYTEXT,"
          + " passwordSalt TINYTEXT, passwordHash TEXT)",
      "CREATE TABLE IF NOT EXISTS permissions (noteID INTEGER, username TINYTEXT, permissionMode"
          + " BIT, FOREIGN KEY(noteID) REFERENCES notes(noteID) ON DELETE CASCADE, FOREIGN"
          + " KEY(username) REFERENCES users(username) ON DELETE CASCADE)"
    };
    for (String s : sql) {
  ...
  /**
   * Close Databse Connection on Shutdown https://stackoverflow.com/questions/64318551
